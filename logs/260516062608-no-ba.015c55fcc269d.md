We need to analyze the login feature. The user wants "Code tính năng đăng nhập" which means "Code login feature". But the instruction is to perform business analysis and technical mapping for a given feature. So we need to produce a full analysis for login.

We'll follow the required output format: TASK, ENTITIES, EXECUTES, MÔ TẢ, TÁC NHÂN, DỮ LIỆU ĐẦU VÀO, QUY TRÌNH THỰC HIỆN, QUY TẮC NGHIỆP VỤ, DỮ LIỆU ĐẦU RA, BUSINESS ANALYSIS STANDARDS (Decision Table, Acceptance Criteria, Domain Model, Test Case Specification), UML & FLOW DIAGRAM (Sequence, State, Flowchart, Class Diagram), TECHNICAL MAPPING (Schemas, Types, Utils, API, Components, Composables, Pages, Middleware).

We'll assume a typical login system with email/password authentication. We'll include edge cases like invalid credentials, locked account, etc.

Let's write the response in Vietnamese as required.
